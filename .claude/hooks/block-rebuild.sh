...
if echo "$COMMAND" | grep -qE '(^|[;&|()]\s*)(sudo\s+)?(just\s+(rebuild|update|rollback)\b|nh\s+(darwin|os)\s+switch\b|(darwin|nixos)-rebuild\s+(switch|boot|test|activate)\b)'; then
    echo "Live-system mutation blocked: 'just rebuild/update/rollback', 'nh … switch', and 'darwin-/nixos-rebuild switch|boot|test|activate' all activate the system, and the user runs those. Validate with 'just check' or 'just eval'; if the change needs applying, give the user the command." >&2
    exit 2
fi
